...
def handle_add(carti):
    try:
        ...
        return create(carti, id_carte, titlu, pret, gen, tip_reducere)
...
def handle_update(carti):
    try:
        ...
        return update(carti, creeaza_carte(id_carte, titlu, pret, gen, tip_reducere))
    except ValueError as ve:
        print('Eroare', ve)
...
def handle_reducere_pret(carti):
    try:
        ...
        carti=reducere_pret_pentru_nume(carti, nume, procentaj)
...
def handle_modify_gen_by_title(carti):
    try:
        titlu=input('Dati titlul cautat pentru schimbarea genului: ')

        carti=modify_gen_by_title(carti, titlu)
...
def run_ui(carti):
    ...
    while True:
        ...
        if optiune == '1':
            ...
            carti = handle_reducere_pret(carti)
        elif optiune == '3':
            carti= handle_modify_gen_by_title(carti)
        elif optiune== '4':
