...
from Userinterface.command_line_console import command_line_console,show_menu_in_line
...
def handle_reducere_pret(carti, undo_list, redo_list):
    try:
        ...
        procentaj=(input('Dati tipul de reducere :'))

        carti=reducere_pret_pentru_nume(carti, [], [])
...
def handle_sort_ascending(carti, undo_list, redo_list):
    carti_aranjate = sort_ascending(carti, undo_list, redo_list)

    return carti_aranjate


def handle_titluri_gen(carti):
    nt, g = distinct_titles(carti)
...
def handle_command_console(carti, undo_list, redo_list):
    return command_line_console(carti, undo_list, redo_list)
...
def run_ui(carti, undo_list, redo_list):
    ...
    while True:
        ...
        if optiune == '1':
            ...
            carti=handle_sort_ascending(carti, undo_list, redo_list)
        elif optiune == '6':
            handle_titluri_gen(carti)
        elif optiune== 'cd':
            show_menu_in_line()
            carti=handle_command_console(carti, undo_list, redo_list)
        elif optiune == 'u':
